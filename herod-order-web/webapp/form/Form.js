...
Ext.define('form.Form', {
	...
	save : function() {
		...
		var actionText = me.nodeId ? '更新' : '新增';
	...
	getFormData : function() {
		var me = this, formValueMap = me.getFormValues(), fieldValues = [];
	...
	uploadImage : function(me, uploadSuccessListener) {
		...
		form.submit({
			...
			success : function(form, action) {
				var images = Ext.JSON.decode(action.response.responseText).images;
				Ext.Msg.alert("提示", "图片上传成功！");
				if (uploadSuccessListener) {
					uploadSuccessListener(images);
				}
			},
